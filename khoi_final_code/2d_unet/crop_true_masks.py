"""
Cropping true masks using xy predicted by yolov5
"""
import sys
...
if __name__ == "__main__": 
    ...
    TRUE = Path(RUNS_YOLOV5) / f"detect/holdout_{args.holdout_fold}_true"
    ...
    for fold in tqdm(range(1,6)):
        ...
        FOLD_TRUE_CROPS = TRUE / f"fold_{fold}/crops/lesion"
        
        if FOLD_TRUE_CROPS.is_dir() != True:
            FOLD_TRUE_CROPS.mkdir()
        ...
        for slice in all_labels:
            mask = read_image(str(Path(AUGMENTED_MASKS) / f"fold_{fold}" / f"{slice.stem}.png"), mode = torchvision.io.ImageReadMode.GRAY)[0]
            ...
            with open(str(slice), "r") as f:
                for count, line in enumerate(f):
                    ...
                    cropped_mask = crop(mask, top = top, left = left, height= HEIGHT, width= WIDTH)
                    plt.imsave(FOLD_TRUE_CROPS / f"{slice.stem}{count + 1 if count > 0 else ''}.png", cropped_mask, cmap = plt.gray())
